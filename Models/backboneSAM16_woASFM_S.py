...
class NoWeightNoSurroundingFusionModule(nn.Module):
    """消融研究：去除权重融合机制并将surrounding设为全零"""
...
class MultiSemanticPromptGenerator(nn.Module):
    ...
    def __init__(self, num_points=20, d_model=256, sam_embed_dim=256, 
        ...
        # 使用去除周围特征和权重融合的模块
        self.fusion_module = NoWeightNoSurroundingFusionModule(clip_dim, sam_embed_dim, d_model)
