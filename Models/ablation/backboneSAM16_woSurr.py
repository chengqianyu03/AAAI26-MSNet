...
class CompactMultiSemanticFusion(nn.Module):
    """简化但保留完整语义分解的融合模块，ablation study版本 - 去除surrounding语义"""
    def __init__(self, clip_dim, sam_dim, d_model=256):
        ...
        # 保留surrounding_encoder保持模型结构一致，但不使用
        ...
        nn.init.constant_(self.weights[0], 0.2)  # RGB权重
        nn.init.constant_(self.weights[1], 0.2)  # 反射权重  
        nn.init.constant_(self.weights[2], 0.3)  # 透射权重
        nn.init.constant_(self.weights[3], 0.2)  # 周围权重
        nn.init.constant_(self.weights[4], 0.1)  # SAM权重
    ...
    def forward(self, rgb_features, reflection_features, sam_features):
        ...
        fused_features = (
            ...
            norm_weights[3] * surrounding_features +  # 零向量会导致这一项不贡献任何信息
