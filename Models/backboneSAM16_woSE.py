...
class CompactMultiSemanticFusion(nn.Module):
    """简化但保留完整语义分解的融合模块，surrounding也用difference encoder"""
    def __init__(self, clip_dim, sam_dim, d_model=256):
        ...
        # 新增：为surrounding特征单独添加一个encoder - 消融研究中仍然保留结构但不使用
        ...
        # 可学习的融合权重 - 修改初始化以适应消融研究
        self.weights = nn.Parameter(torch.ones(5, 1))  # RGB, 反射, 透射, 周围, SAM
        nn.init.constant_(self.weights[0], 0.15)  # RGB权重增加
        nn.init.constant_(self.weights[1], 0.15)  # 反射权重增加 
        nn.init.constant_(self.weights[2], 0.2)   # 透射权重设为0
        nn.init.constant_(self.weights[3], 0.2)   # 周围权重设为0
        nn.init.constant_(self.weights[4], 0.3)   # SAM权重
    ...
    def forward(self, rgb_features, reflection_features, sam_features):
        ...
        # 消融研究：创建空特征（零张量）替代透射和周围特征
        zero_features = torch.zeros_like(rgb_proj)
        
        # 1. 透射特征设为零张量
        trans_features = zero_features
        
        # 2. 周围语义特征也设为零张量
        surrounding_features = zero_features
        
        # 3. 生成透射注意力图 - 使用零张量生成（近似于0.5的常值）
        diff_attention = self.attention_gen(zero_features)
        ...
        # 记录权重值
        weights = {
            "rgb_weight": norm_weights[0].item(),
            "reflection_weight": norm_weights[1].item(),
            "transmission_weight": norm_weights[2].item(),
            "surrounding_weight": norm_weights[3].item(),
            "sam_weight": norm_weights[4].item()
        }
