...
class NoTransNoSurroundNoASFMFusionModule(nn.Module):
    """消融研究：去除ASFM、transmission和surrounding均设为全零"""
    ...
    def forward(self, rgb_features, reflection_features, sam_features):
        ...
        # 1. 计算透射特征 - 但忽略结果，使用零向量
        trans_features_real = torch.abs(rgb_proj - refl_proj)
        trans_features_real = self.difference_encoder(trans_features_real)
        # 生成透射注意力图 - 使用真实trans_features以保持一致性
        diff_attention = self.attention_gen(trans_features_real)
        
        # 对于消融实验，使用零向量替代透射特征
        trans_features = torch.zeros_like(rgb_proj)
        
        # 2. 周围特征: 使用零向量
        surrounding_features = torch.zeros_like(rgb_proj)
        
        # 拼接所有特征而不是加权融合
        concat_features = torch.cat([
            rgb_proj,                # RGB特征
            refl_proj,               # 反射特征
            trans_features,          # 透射特征 (全零)
            surrounding_features,    # 周围特征 (全零)
            sam_proj                 # SAM特征
        ], dim=1)
        ...
        weights = {
            "rgb_weight": 0.333,
            "reflection_weight": 0.333,
            "transmission_weight": 0.0,  # 显式标记为0
            "surrounding_weight": 0.0,   # 显式标记为0
            "sam_weight": 0.333
        }
...
class MultiSemanticPromptGenerator(nn.Module):
    """使用多语义分解的简化提示生成器 - 去除transmission和surrounding版本"""
    def __init__(self, num_points=20, d_model=256, sam_embed_dim=256, 
        ...
        # 使用去除transmission、surrounding特征和ASFM的融合模块
        self.fusion_module = NoTransNoSurroundNoASFMFusionModule(clip_dim, sam_embed_dim, d_model)
